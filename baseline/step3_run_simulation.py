...
_MACHINE_NUM_DICT = {"A100": 1, "RTX5090": 2}
...
_ILP_DIR = os.path.join(_BASELINE_DIR, "layouts", "ilp")
_ILP_SOL = os.path.join(_ILP_DIR, "ilp_sol.ini")
_SIM_CLUSTER_INI = os.path.join(_ILP_DIR, "simulator_cluster.ini")
_SIM_FILES = os.path.join(_BASELINE_DIR, "sim_files")


def _workspace_dir(*parts: str) -> str:
    d = os.path.join(_SIM_FILES, *parts)
    os.makedirs(d, exist_ok=True)
    return d + os.sep


def _baseline_tmp_dir() -> str:
    d = os.path.join(_BASELINE_DIR, "tmp")
    os.makedirs(d, exist_ok=True)
    return d + os.sep


def _sim_files_root() -> str:
    os.makedirs(_SIM_FILES, exist_ok=True)
    return _SIM_FILES + os.sep
...
def simulate_maxflow_offline():
    ...
    machine_num_dict = _MACHINE_NUM_DICT
    layout_synthesizer = LayoutSynthesizer(
        ...
        workspace_path=_workspace_dir("maxflow_offline"),
        layout_method=LayoutMethod.LoadExisting,
        machine_num_dict=machine_num_dict
    )
    layout_args = {
        "solution_file_name": _ILP_SOL,
        "simulator_cluster_file_name": _SIM_CLUSTER_INI,
    }
    ...
    simulator.visualize_cluster(title="model_placement", save_path=_workspace_dir("maxflow_offline"))
    ...
    print(f"# ------------------------------------------------------------- #")
    simulator.plot_inference_speed(max_time=700, save_path=os.path.join(_SIM_FILES, "maxflow_offline", "throughput.png"))
    simulator.plot_request_latency(ignore_initialize=True, save_path=os.path.join(_SIM_FILES, "maxflow_offline", "latency.png"))
...
def simulate_maxflow_online(avg_throughput: float):
    ...
    layout_synthesizer = LayoutSynthesizer(
        ...
        machine_profile_name=_MACHINE_PROFILE,
        model_name=ModelName.LLaMa70B,
        workspace_path=_workspace_dir("maxflow_online"),
        layout_method=LayoutMethod.LoadExisting,
        machine_num_dict=machine_num_dict
    )
    layout_args = {
    ...
    simulator.visualize_cluster(title="model_placement", save_path=_workspace_dir("maxflow_online"))
    ...
    print(f"Avg decode latency: {avg_decode_latency:.3f}s")
    print(f"# ------------------------------------------------------------- #")
    simulator.plot_inference_speed(max_time=700, save_path=os.path.join(_SIM_FILES, "maxflow_online", "throughput.png"))
    simulator.plot_request_latency(ignore_initialize=True, save_path=os.path.join(_SIM_FILES, "maxflow_online", "latency.png"))

    # save the latency data with pickle
    import pickle
    prompt_file_name = os.path.join(_SIM_FILES, "maxflow_online", "prompt_latency.pkl")
    decode_file_name = os.path.join(_SIM_FILES, "maxflow_online", "decode_latency.pkl")
...
def simulate_heuristic_offline(scheduling_method: SchedulingMethod, initial_query_num: int):
    ...
    layout_synthesizer = LayoutSynthesizer(
        ...
        machine_profile_name=_MACHINE_PROFILE,
        model_name=ModelName.LLaMa70B,
        workspace_path=_baseline_tmp_dir(),
        layout_method=LayoutMethod.LoadExisting,
        machine_num_dict=machine_num_dict
    )
    layout_args = {
...
def simulate_heuristic_online(scheduling_method: SchedulingMethod, avg_throughput: float):
    ...
    layout_synthesizer = LayoutSynthesizer(
        ...
        machine_profile_name=_MACHINE_PROFILE,
        model_name=ModelName.LLaMa70B,
        workspace_path=_sim_files_root(),
        layout_method=LayoutMethod.LoadExisting,
        machine_num_dict=machine_num_dict
    )
    layout_args = {
