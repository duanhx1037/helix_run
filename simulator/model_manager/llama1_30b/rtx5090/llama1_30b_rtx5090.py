...
class LLaMa30BonRTX5090(ModelOnMachine):
    def __init__(self, num_machines_dict: Dict[str, int], typical_layers_dict: Dict[str, int],
                 normalized_perf_dict: Dict[str, float]):
        """
        LLaMa1-30B + RTX 5090 32GB

        Latency tables: prompt_bs2time.csv, decode_bs2time.csv in this directory.

        32GB 单卡：显存优于 L4(24GB)，prefill 并发上界对齐 llama1_30b_l4（各层 3），而非 T4x2 的全 1。
        max_num_layers / num_gpu_blocks：5090 probe（dummy）锚点 15/22/24；1–14 与 T4x2 表连续；16–21、23 插值；
        decode 上界随层数递减，为启发式占位。
        """
        # --------------- Machine Dependent Data --------------- #
        # STEP 1/3: create prompt_bs2time.csv and decode_bs2time.csv
        # STEP 2/3: some basic parameters
        machine_name: str = "RTX5090"
        max_num_layers: int = 24
        # STEP 3/3: inference settings
        # num_gpu_blocks: 1–14 与 T4x2 同形状；15/22/24 来自 5090 probe；16–21、23 为插值
        vllm_num_blocks_dict: Dict[int, int] = {
            1: 63612, 2: 30419, 3: 19440, 4: 13951, 5: 10657, 6: 8461, 7: 6893, 8: 5717, 9: 4802, 10: 4070, 11: 3471, 12: 2972, 13: 2550, 14: 2188,15: 1874, 16: 1600, 17: 1357, 18: 1142,19: 950, 20: 776, 21: 619, 22: 477, 23: 347, 24: 227
        ...
        decode_max_tokens_dict: Dict[int, int] = {
            ...
            13: 35, 14: 25, 15: 20, 16: 18, 17: 17, 18: 16, 19: 15, 20: 14, 21: 12, 22: 11, 23: 10, 24: 9,
        }
        # ------------------------------------------------------ #
